'''
Experiment with greedy graph construction. Construct greedily, beam search k steps in arrows, 
...
from src.abapc import get_cg_and_facts, score_model_original, score_model_by_refined_indep_facts
...
SAMPLE_SIZE = 5000
RESULT_DIR = Path("./results/gradual/stand_alone_gradual_v1")
    ...
    for dataset in DATASETS:
        ...
        for seed in tqdm(seeds_list, desc=f"Running {dataset} with {n_runs} seeds"):
            ...
            score_original_prefilled = partial(score_model_original,
                                               ...
                                               alpha=ALPHA,
                                               return_only_I=True)
            score_refined_prefilled = partial(score_model_by_refined_indep_facts,
            ...
            start_refined_ranking = time.time()
            ...
            for best_I, best_model, elapsed, method in [
                (original_ranking_I, best_model_original_ranking, elapsed_orig_ranking, 'original_ranking'),
                (refined_ranking_I, best_model_refined_ranking, elapsed_refined_ranking, 'refined_ranking'),
            ]:                    
                # get metrics
