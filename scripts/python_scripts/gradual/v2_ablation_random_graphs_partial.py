...
SAMPLE_SIZE = 5000
RESULT_DIR = Path("./results/gradual/v2_ablation_random_graphs_partial")
...
def get_param_sets():
    """
    Generate parameter sets by varying one parameter at a time,
    holding others at their maximum/default values.
    """
    param_sets = []

    # Define max/default values
    defaults = {
        "neighbourhood_n_nodes": max(NEIGHBOURHOOD_N_NODES),
        "c_set_size": max(C_SET_SIZE),
        "use_collider_arguments": True,
    }

    for n_nodes in N_NODES:
        # Base configuration with all default values
        base_params = (
            n_nodes,
            defaults["neighbourhood_n_nodes"],
            defaults["c_set_size"],
            defaults["use_collider_arguments"],
        )
        param_sets.append(base_params)

        # Vary each parameter individually (excluding defaults)
        for nn in NEIGHBOURHOOD_N_NODES:
            if nn != defaults["neighbourhood_n_nodes"] and nn <= n_nodes:
                param_sets.append((
                    n_nodes,
                    nn,
                    defaults["c_set_size"],
        ...
        for cs in C_SET_SIZE:
            if cs != defaults["c_set_size"]:
                param_sets.append((
                    n_nodes,
                    defaults["neighbourhood_n_nodes"],
                    cs,
        ...
        for ca in [True, False]:
            if ca != defaults["use_collider_arguments"]:
                param_sets.append((
                    n_nodes,
                    defaults["neighbourhood_n_nodes"],
                    defaults["c_set_size"],
...
def main(n_runs):
    ...
    for param_set in param_sets:
        ...
        # check if this set of parameters was already processed before creating BSAF
        any_param_set_not_processed = False
        for seed in seeds_list:
            for search_depth in SEARCH_DEPTH:
                ...
                if run_params not in processed:
                    any_param_set_not_processed = True

        if any_param_set_not_processed:
            logger.info(f"Processing new parameter set: {param_set}")
            # create bsaf
            ...
            for seed in tqdm(seeds_list, desc=f"Running {dataset} with {n_runs} seeds"):
                ...
                if any_search_depth_not_explored:
                    logger.info(f"Running for seed: {seed}, search_depth: {search_depth}")
