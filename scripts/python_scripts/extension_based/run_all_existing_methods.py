"""
run all existing methods: random, mpc, nt, fgs for up to child 20 nodes.
run abapc original for up to sachs 11 nodes.
save all results.
"""
...
def main(n_runs=50, sample_size=5000, device=0):
    ...
    for dataset_name in tqdm(dataset_list, desc="tqdm Datasets"):
        for method in tqdm(model_list, desc="tqdm Methods"):
            ...
            if method == 'abapc' and dataset_name in ['asia', 'sachs', 'child', 'insurance']:
