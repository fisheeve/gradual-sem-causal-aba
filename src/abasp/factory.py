...
class ABASPSolverFactory(CoreABASPSolverFactory):
    ...
    @staticmethod
    def _add_dependence_rules(solver, path_id, path_nodes, X, Y, S):
        ...
        for (X, Y) in node_pairs_considered:
            ...
            for S in powerset(set(range(self.n_nodes)) - {X, Y}):
                ...
                for path_id, my_path in enumerate(paths):
                    ...
                    # active path rule
                    self._add_dependence_rules(solver, path_id, my_path, X, Y, S)

        # finally add facts
        self._add_facts(solver, facts)
